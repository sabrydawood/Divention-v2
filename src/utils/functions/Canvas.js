...
//const ProfileBadges = require('./ProfileBadges');
...
module.exports = class CanvasUtil {
  ...
  static RainbowColorPercentage(percentage) {
    ...
    const canvas = createCanvas(456, 75);
    ...
    // Rainbow ship
    context.fillRect(0, 0, canvas.width * howMuchToFill, canvas.height, true, false);

    // BlackBox
    context.fillRect(0, 0, canvas.width, canvas.height, false, true);

    return canvas.toDataURL();
  }

  static async ShipImage(shipValue, member1, member2) {
    const canvas = createCanvas(512, 350);
    const ctx = canvas.getContext('2d');
    ...
    const avatarOneLoaded = await loadImage(linkFirstAvatar);
    const avatarTwoLoaded = await loadImage(linkSecondAvatar);
    const shipLoadedImage = await loadImage(this.RainbowColorPercentage(shipValue));
